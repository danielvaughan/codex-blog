...
A regex-based validator checks for empty output, accidental code blocks, and missing structural markers. A token-budget guard estimates the rewritten prompt's cost (1 token per non-ASCII character, 0.25 per ASCII character) and reverts to the original when the rewrite fails to achieve at least a 5% reduction[^1]. This fallback prevents the middleware from ever making things worse.
...
Across three commercial backends, the middleware delivers consistent prompt compression while maintaining or improving accuracy:
...
The total-token savings are lower than prompt savings because some backends generate longer answers from cleaner prompts — GPT-3.5-turbo's answer tokens increase by 42.3%[^1]. This is the accuracy-verbosity trade-off: cleaner input yields more complete output.

### The Function-Name Oracle Ablation

Stripping the SLM rewrite and supplying only regex-extracted function names reveals that the translation step drives the accuracy gain, not the oracle alone. GPT-3.5-turbo drops to 79.0% (−20.5 pp) and Gemini-2.5-flash-lite to 76.5% (−18.5 pp) without the full rewrite pipeline[^1]. GPT-4o is robust (+0.17 pp), suggesting that stronger models compensate for tokenisation noise at the cost of more compute.

### Versus LLMLingua-2

At a matched 40% compression target, the middleware outperforms LLMLingua-2 on all three backends. The gap is dramatic on weaker models: OckScore 99.08 versus 76.91 on GPT-3.5-turbo, and 97.54 versus 33.85 on Gemini-2.5-flash-lite[^1]. LLMLingua-2's compression latency is also 3.9× higher (689 ms versus 176 ms on identical hardware)[^1].
...
The paper's dollar-cost analysis reveals a counterintuitive result: preprocessing does not always save money. On GPT-3.5-turbo, the longer answers generated from cleaner prompts actually increase total cost by 15.1%. On GPT-4o, savings are negligible (−0.4%). Only on Gemini-2.5-flash-lite is the saving material (−12.4%)[^1]. The real value is not in raw API cost reduction but in **context window efficiency** — fitting more useful information into the same token budget before compaction fires.
