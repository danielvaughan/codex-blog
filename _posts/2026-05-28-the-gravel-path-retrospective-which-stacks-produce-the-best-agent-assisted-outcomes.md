...
**The first enterprise-scale field study** of command-line coding agent adoption (Murphy-Hill, Butler and Savelieva, Microsoft, July 2026) measured tens of thousands of engineers during Microsoft's early-2026 rollout of both Claude Code and GitHub Copilot CLI. Adopters merged roughly **24 per cent more pull requests** than they would have otherwise — a sustained productivity signal across the full four-month observation window. Adoption spread primarily through social networks rather than formal rollout, confirming peer visibility as the dominant lever. The study's most significant finding for Gravel Path practitioners is the cost caveat: token spend at enterprise scale "can run into millions of dollars annually," and the paper warns that at enterprise scale, token spend "can run into millions of dollars annually." The 24 per cent PR uplift is real; the ROI evaporates without cost governance. A harness that controls token spend — through precise AGENTS.md scope, hermetic test feedback that terminates unproductive loops early, and PostToolUse ratchets that catch errors before agents enter retry cycles — is not optional at enterprise scale: it is the mechanism that converts the 24 per cent productivity gain from a cost centre into a net positive.[^microsoft-cli-study]

**A September 2026 contamination-controlled study** added important nuance to the harness effect. "Harness or Model? Isolating the Harness Effect in Agentic Coding with a Contamination-Controlled Private Suite" (arXiv:2609.11987, 8 September 2026) used 256 tasks comprising repository problems and post-cutoff contest problems — designed to eliminate benchmark leakage — to measure harness effects with controlled validity. The headline finding challenges assumptions about vendor-native pairings: comparing identical models (Claude Opus 4.8 and GPT-5.5) run under their native harnesses versus a neutral harness, the researchers found no clear average advantage for the native configuration (−1.25 pp for Claude, +1.25 pp for GPT-5.5). The effect is task-type-dependent: native harnesses showed a 9.0 pp disadvantage on repository tasks but a 23.7 pp advantage on contest-style tasks — though the authors caution this partition was identified post-hoc and requires designed replication. The cost finding is contested: the native harness appeared 1.2–1.6× more cost-efficient per solved task for Opus 4.8, though the authors flag this as unresolved — missing usage records on the Anthropic account mean the true cost ratio could range from 0.7 to 2.3 depending on how the unattributed spend is allocated. For Gravel Path practitioners, the implication is precise: the four artefacts are most valuable not for maximising benchmark scores on contest-style problems but for controlling cost, reducing verification overhead, and maintaining consistent quality on the repository-style tasks that constitute the majority of real-world development work. The Gravel Path's minimal viable harness is optimised exactly for this task profile.[^harness-or-model]
...
*Updated 17 September 2026: added Murphy-Hill, Butler and Savelieva's enterprise field study (arXiv:2607.01418, Microsoft, July 2026) — the first large-scale measurement of command-line coding agent adoption at enterprise scale, showing 24 per cent more merged PRs but with a critical cost caveat: token spend "can run into millions of dollars annually" (the paper's direct cost caveat), confirming that harness cost governance is not optional at enterprise scale. Added "Harness or Model?" (arXiv:2609.11987, September 2026) — a contamination-controlled study finding no clear average advantage for vendor-native harness-model pairings, with the harness effect varying sharply by task type; the cost ordering between harnesses is unresolved due to missing usage data, but the findings validate the Gravel Path's focus on minimal, cost-governed artefacts over vendor-native feature sets.*
...
[^microsoft-cli-study]: Murphy-Hill, E., Butler, J. and Savelieva, A. "Adoption and Impact of Command-Line AI Coding Agents: A Study of Microsoft's Early 2026 Rollout of Claude Code and GitHub Copilot CLI." arXiv:2607.01418, 1 July 2026. Field study of tens of thousands of Microsoft engineers over a four-month observation window. Adopters merged ~24% more pull requests than they would have otherwise; the productivity signal was sustained and showed a dose-response curve. Adoption spread primarily through social networks, not formal rollout channels. The paper's key cost caveat: token spend "can run into millions of dollars annually," making cost governance essential at enterprise scale. Retention correlated more strongly with engineers' existing coding activity than with demographics. https://arxiv.org/abs/2607.01418

[^harness-or-model]: "Harness or Model? Isolating the Harness Effect in Agentic Coding with a Contamination-Controlled Private Suite." arXiv:2609.11987, 8 September 2026. Compared Claude Opus 4.8 and GPT-5.5 under native vs neutral harnesses using 256 contamination-controlled tasks (repository problems + post-cutoff contest problems). Key findings: no clear average advantage for vendor-native pairings (−1.25 pp Claude, +1.25 pp GPT-5.5); the harness effect is task-type-dependent (−9.0 pp on repository tasks, +23.7 pp on contest tasks for the native harness, p=0.003 — but the partition was post-hoc and requires designed replication); native harness appeared 1.2–1.6× more cost-efficient per solved task for Opus 4.8, though the authors flag the cost ordering as unresolved — missing usage data means the true Opus ratio could range from 0.7 to 2.3. Implication: the harness effect is most reliable on real-world repository tasks; cost claims from this study should be treated as preliminary pending replication with complete usage data. Exactly the profile the Gravel Path's four artefacts are designed for. https://arxiv.org/abs/2609.11987
