...
That is a **40-point perception gap**. Developers felt significantly faster while actually being significantly slower. The AI output volume — the raw quantity of code produced — created a sensation of productivity that the actual task completion time did not support. The downstream costs are concrete: the Harness 2025 State of Software Delivery Report found that **67% of developers** spent more time debugging AI-generated code than they would have spent writing it manually, and **68%** spent more time fixing AI-created security issues.[^49]
