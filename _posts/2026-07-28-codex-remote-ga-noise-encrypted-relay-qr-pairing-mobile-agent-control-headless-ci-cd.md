...
Both sides derive directional AES-256-GCM keys via HKDF-SHA256. Every message is wrapped in an encrypted envelope with monotonic counters for replay protection [^2]. The initiator (your phone or a second Codex instance) already knows the host's static public key from the pairing step, which is what makes the one-RTT handshake possible and prevents unknown-host attacks.
